...
class GcategoryApp extends BackendApp
{
	...
	$city=$priv_row['city'];
	 ...
	 $result=$this->kaiguan_mod->getAll("select cate_id from ".DB_PREFIX."gcategory where siteids like '%|$city,%'");
	  $str='';
	  ...
	  $gcate=$this->kaiguan_mod->getRow("select gcategory from ".DB_PREFIX."kaiguan");
	  $this->assign('gcate', $gcate);
        /* 取得商品分类 */
        $gcategories = $this->_gcategory_mod->get_list(0);
        $tree =& $this->_tree($gcategories);

		$result=$this->_gcategory_mod->getAll("select * from ".DB_PREFIX."city");
		foreach ($result as $var )
		{
        ...
        foreach ($gcategories as $key => $val)
        {
			...
			$gcategories[$key]['city_name'] = $city_row[$val['city']];
			
			if(strpos($cate_ids,$val['cate_id'])!==false)
			$gcategories[$key]['checked']=1; 
        }
	 ...
	 else
	 {
	  $city_row=$this->city_mod->getAll("select * from ".DB_PREFIX."city where city_id='$cityid'");
        ...
        else
        {
            ...
            /* 检查名称是否已存在 */
            if (!$this->_gcategory_mod->unique(trim($data['cate_name']), $data['parent_id'],'',$data['city']))
            {
...
$privs=$priv_row['privs'];
$cityid=$priv_row['city'];
$gcater=$this->userpriv_mod->getRow("select city from ".DB_PREFIX."gcategory where cate_id = '$id' limit 1");
	if($privs=="all")
	 {
	 $city_row=$this->city_mod->getAll("select * from ".DB_PREFIX."city");
	 }
    ...
    function batch_edit()
    {
	  $id1 = isset($_GET['id']) ? trim($_GET['id']) : '';
	  $ids1 = explode(',', $id1);
        if (!IS_POST)
        {
		$userid=$this->visitor->get('user_id');
	 ...
	 $this->assign('priv_row', $priv_row);
		 foreach ($ids1 as $v)
		 {
		$isxianshi=$this->userpriv_mod->getrow("select siteids from ".DB_PREFIX."gcategory where cate_id = '$v' and store_id=0");
		//print_r("select is_xianshi from ".DB_PREFIX."gcategory where cate_id = '$v' and store_id=0");
		$str='';
		$isxian=substr($isxianshi['siteids'],1);
		$newstr = substr($isxian,0,strlen($isxian)-1); 
		$yh=explode(', |',$newstr);
		foreach ($city_row as $val)
		{
			$che='';
			$shu='|';
			...
			else if($priv_row['city']!=$val['city_id'])
			{
				$dv='<div style="display:none">';
				$dv1='</div>';
			}
			else if($priv_row['city']==$val['city_id'])
			{
			...
			$str.= $dv.'<input type="checkbox" name="fu[]" value="'.$shu.''.$val['city_id'].'" '.$che.'/>'.$val['city_name']. $dv1;
			$this->assign('xianshi', $str);
		}
