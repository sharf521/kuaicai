...
class BrandApp extends BackendApp
{
	...
	$priv_row=$this->_admin_mod->getRow("select * from 

".DB_PREFIX."user_priv where user_id = '$userid' and store_id=0 limit 1");
	...
	$this->assign('priv_row', $priv_row);
		...
		{
		$brands=$this->_brand_mod->find(array(
        'conditions'    => '1=1 and city='.$city . $conditions . $verify,
        'limit'         => $page['limit'],
        'order'         => "$sort $order",
        'count'         => true
        ));
        ...
        foreach ($brands as $key => $brand)
        {
			...
			$brands[$key]['city_name'] = $city_row[$brand['city']];
        }
	 ...
	 else
	 {
	  $city_row=$this->city_mod->getAll("select * from ".DB_PREFIX."city where city_id='$cityid'");
		...
		$city=$_POST['city'];
            ...
            /* 检查名称是否已存在 */
            if (!$this->_brand_mod->unique(trim($data['brand_name']),'',$city))
            {
...
$privs=$priv_row['privs'];
$cityid=$priv_row['city'];
$br=$this->_admin_mod->getRow("select city from ".DB_PREFIX."brand where brand_id = '$brand_id' limit 1");
	if($privs=="all")
	 {
	 $city_row=$this->city_mod->getAll("select * from ".DB_PREFIX."city");
	 }
	  ...
	  $city_row=$this->city_mod->getAll("select * from ".DB_PREFIX."city where city_id='$cityid'");
		...
		$city=$_POST['city'];
            ...
            if ($logo === false)
            {
             ...
             /* 检查名称是否已存在 */
            if (!$this->_brand_mod->unique(trim($data['brand_name']), $brand_id,$city))
            {
                $this->show_warning('name_exist');
                return;
            }
            ...
            $this->show_message('edit_brand_successed',
                'back_list',        'index.php?app=brand&page= '. $pag,
