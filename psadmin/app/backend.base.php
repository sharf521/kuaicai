...
class BackendApp extends ECBaseApp
{
    ...
    { 
        ...
        $user_info = $mod_user->get(array(
            'conditions' => $user_id,
            'join'       => 'manage_mall',
            'fields'     => 'this.user_id, user_name, reg_time, last_login, last_ip, privs, this.city'
        ));
...
$my_money_row=$mod_user->getAll("select * from ".DB_PREFIX."my_money where user_id='$user_id'");
if(empty($my_money_row))
{
$member_row=$mod_user->getRow("select * from ".DB_PREFIX."member where user_id='$user_id' limit 1");
