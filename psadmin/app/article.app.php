...
$userid=$this->visitor->get('user_id');
	...
	$this->assign('priv_row', $priv_row);
		...
		$articles=$this->_article_mod->find(array(
        'fields'   => 'article.*,acategory.cate_name',
        'conditions'  => 'store_id=0 and city='.$city . $conditions,
        'limit'   => $page['limit'],
        'join'    => 'belongs_to_acategory',
        'order'   => 'article.sort_order ASC,article.add_time DESC', //必须加别名
        'count'   => true   //允许统计
		...
		$city_row=array();
		$result=$this->_article_mod->getAll("select * from ".DB_PREFIX."city");
		 ...
		 foreach ($articles as $key => $val)
        {
			...
			$articles[$key]['city_name'] = $city_row[$val['city']];	
        }	
	...
	$city=$priv_row['city'];
		...
		if($privs=="all")
		{
		$city_row=$this->_city_mod->getAll("select * from ".DB_PREFIX."city");
		}
		else
		{
		$city_row=$this->_city_mod->getAll("select * from ".DB_PREFIX."city where city_id='$city'");
		}
            /* 显示新增表单 */
	...
	$privs=$priv_row['privs'];
	$city=$priv_row['city'];
	$artic=$this->userpriv_mod->getRow("select city from ".DB_PREFIX."article where article_id = '$article_id' limit 1");
        if (!$article_id)
        {
        ...
        else
        {
            ...
            $this->show_message('edit_article_successed',
                'back_list',        'index.php?app=article&page= '. $pag,
   ...
   function ajax_col()
   {
    ...
    function drop_uploadedfile()
    {
        ...
        if ($file_id && $this->_uploadedfile_mod->drop1($file_id))
        {
