...
class SgradeApp extends BackendApp
{
    ...
    function SgradeApp()
    {
		...
		$this->priv_mod = & m('userpriv');
    }

    function index()
    {
	$userid=$this->visitor->get('user_id');
$priv_row=$this->priv_mod->getRow("select * from ".DB_PREFIX."user_priv where user_id = '$userid' and store_id=0 limit 1");
	...
	function storelog()
	{
		...
		if($storename!="")
		{
			$conditions.=" and store_name like '%$storename%'";
		}
		...
		$fangshi = trim($_GET['fangshi']);
		$this->assign('fangshi', $fangshi);
		if($fangshi!="")
		{
			$conditions.=" and gettype='$fangshi'";
		}
		$index=$this->storelog_mod->getAll("SELECT * " .
                    "FROM " . DB_PREFIX . "storelog " .
					" where " . $conditions .
					" order by  id desc " .
                    " LIMIT {$page['limit']} "
					);
		...
		$city_row=array();
		$result=$this->storelog_mod->getAll("select * from ".DB_PREFIX."city");
		...
		$qian=$this->storelog_mod->getAll("SELECT * " .
                    "FROM " . DB_PREFIX . "storelog " .
					" where " . $conditions 
					);
        ...
        $this->display('storelog.index.html');
	...
	function xiaobaolog()
	{
		...
		$index=$this->xiaobaolog_mod->getAll("SELECT * " .
                    "FROM " . DB_PREFIX . "xiaobaolog " .
					" where " . $conditions .
					" order by  id desc " .
                    " LIMIT {$page['limit']} "
					);
		...
		$city_row=array();
		$result=$this->xiaobaolog_mod->getAll("select * from ".DB_PREFIX."city");
		foreach ($result as $var )
		{
		  $row=explode('-',$var['city_name']);
		  $city_row[$var['city_id']]=$row[0];
		...
		$qian=$this->xiaobaolog_mod->getAll("SELECT * " .
                    "FROM " . DB_PREFIX . "xiaobaolog " .
					" where " . $conditions 
					);
		$page['item_count'] = count($qian);
        ...
        $this->assign('page_info', $page);
        $this->assign('index', $index);//传递到风格里
        $this->display('xiaobaolog.index.html');
