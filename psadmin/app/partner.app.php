...
class PartnerApp extends BackendApp
{
	...
	$this->assign('priv_row', $priv_row); 
		...
		{
		$partners = $this->_partner_mod->find(array(
            'conditions'    => 'store_id=0 and pcity='.$city . $conditions,
            'limit'         => $page['limit'],  //获取当前页的数据
            'order'         => 'sort_order,partner_id ASC',
            'count'         => true             //允许统计
        )); //找出所有商城的合作伙伴
		...
		$city_row=array();
		$result=$this->_partner_mod->getAll("select * from ".DB_PREFIX."city");
        ...
        foreach ($partners as $key => $partner)
        {
			...
			$partners[$key]['city_name'] = $city_row[$partner['pcity']];	
        }
	...
	$priv_row=$this->userpriv_mod->getRow("select * from ".DB_PREFIX."user_priv where user_id = '$userid' and store_id=0 limit 1");
	$cityid=$priv_row['city'];
	$part=$this->userpriv_mod->getRow("select pcity from ".DB_PREFIX."partner where partner_id = '$partner_id' limit 1");
	 if($priv_row['privs']=="all")
	 {
	   $city_row=$this->city_mod->getAll("select * from ".DB_PREFIX."city");
	  }
	  ...
	  $city_row=$this->city_mod->getAll("select * from ".DB_PREFIX."city where city_id='$cityid'");
        ...
        else
        {
            ...
            $this->show_message('edit_partner_successed',
				'back_list',     'index.php?app=partner&page= '. $pag,
