/* spec对象 */
function spec(id, spec1, spec2, price, stock,jifen_price,vip_price)
{
    this.id    = id;
    this.spec1 = spec1;
    this.spec2 = spec2;
...
function selectSpec(num, liObj)
{
    ...
    else
    {
        ...
        if (spec != null)
        {
            $("[ectype='current_spec']").html(spec.spec1 + ' ' + spec.spec2);
			if(daishou==3)
			$price_m=Math.ceil(spec.price*100)/100;
			else
			$price_m=Math.ceil(spec.price*121)/100;
            $("[ectype='goods_price']").html(price_format($price_m));
			$("[ectype='goods_jifen_price']").html((spec.jifen_price+'积分'));
			$("[ectype='goods_vip_price']").html((spec.vip_price+'积分'));
