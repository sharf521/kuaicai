...
function spec_update(){
    ...
    SPEC.specs && $.each(SPEC.specs,function(i,item){
        ...
        tpl.find('*[item="price"]').append('<input type="hidden" name="price['+ item.spec_id +']" value="' + item.price + '" />' + item.price);
		 tpl.find('*[item="jifen_price"]').append('<input type="hidden" name="jifen_price['+ item.spec_id +']" value="' + item.jifen_price + '" />' + item.jifen_price);
		  tpl.find('*[item="vip_price"]').append('<input type="hidden" name="vip_price['+ item.spec_id +']" value="' + item.vip_price + '" />' + item.vip_price);
...
function spec_editor(){
    ...
    spec_item && $.each(spec_item,function(i,item){ // 遍历生成规格项
		...
		tpl.find('*[item="jifen_price"]').val(item.jifen_price);
		tpl.find('*[item="vip_price"]').val(item.vip_price);
    ...
    $('*[ectype="save_spec"]').click(function(){
        ...
        $('#dialog_object_spec_editor').find('*[ectype="data"]').each(function(){
			...
			var jifen_price=price*bili*(1+lv31);
			var vip_price =price*bili*(1+lv21);
			/*var jifen_price = $.trim($(this).find('*[item="jifen_price"]').val());

			var vip_price = $.trim($(this).find('*[item="vip_price"]').val());*/
            ...
            if(SPEC.spec_qty == 1){ // 一个规格
                ...
                eval('if(spec_' + spec_pos + ' || (!spec_' + spec_pos + ' && !price && !jifen_price && !vip_price && !stock && !sku)){}else{complate = false;}');
            }else{ // 两个规格
                if((spec_1 && spec_2 && price && stock && sku) || (!spec_1 && !spec_2 && !price && !jifen_price && !vip_price && !stock && !sku)){
            ...
            item != ';' && SPEC.specs.push({
				...
				'jifen_price':number_format(jifen_price, 5),
				'vip_price':number_format(vip_price, 5),
