            ...
            $.get(selfurl + "&act=ajax_cate", {id: id}, function(data){
                if(data)
                {
                    ...
                    for(var i = 0; i < res.length; i++)
                    {
						...
						if(cate_ids.indexOf(','+res[i].cate_id+',')>0)
						{
							str+="<tr class='row"+id+"'><td class='align_center w30'><input type='checkbox' class='checkitem' value='"+res[i].cate_id+"' checked/></td>";
						}
						else
						{
                        ...
                        str+="<td class='node' width='50%'><img class='preimg' src='templates/style/images/treetable/vertline.gif'/>"+src+"<span class='node_name ' ectype='inline_edit' fieldname='cate_name' fieldid='"+res[i].cate_id+"' required='1'>"+res[i].cate_name+"</span></td>";
			if(guanli=="all")
			{
            str+="<td class='align_center'><span class='editable' ectype='inline_edit' fieldname='sort_order' fieldid='"+res[i].cate_id+"' datatype='number'>"+res[i].sort_order+"</span></td>";
			}
            //str+="<td class='align_center'>"+if_show+"</td>";
			if(guanli=='all')
			{
            str+="<td class='handler'><span><a href='index.php?app=gcategory&amp;act=edit&amp;id="+res[i].cate_id+"'>"+lang.edit+"</a>  | <a href='javascript:if(confirm(\""+lang.confirm_delete+"\"))window.location=\"index.php?app=gcategory&act=drop&id="+res[i].cate_id+"\";'>"+lang.drop+"</a> | <a href='index.php?app=gcategory&amp;act=add&amp;pid="+res[i].cate_id+"'>"+lang.add_child+"</a></span></td></tr>";
			}
                    }
                    //将组成的字符串添加到点击对象后面
                    pr.after(str);
    ...
    var src = $(ob).attr('src');
        if(status == 'open')
        {
            ...
            $.get(selfurl + "&act=ajax_cate", {id: id}, function(data){
                if(data)
                {
                    ...
                    for(var i = 0; i < res.length; i++)
                    {
                        ...
                        {
                           src =  "<img src='templates/style/images/treetable/tv-expandable.gif' ectype='flex' status='open' fieldid="+res[i].cate_id+
                           " onclick='secajax($(this))'><span class='node_name ' ectype='inline_edit' fieldname='cate_name' fieldid='"+res[i].cate_id+"' required='1'>"+res[i].cate_name+"</span>";
                           
                        }
                        else
                        {
                           src =  "<img src='templates/style/images/treetable/tv-item.gif' fieldid='"+res[i].cate_id+"'><span class='node_name ' ectype='inline_edit' fieldname='cate_name' fieldid='"+res[i].cate_id+"' required='1'>"+res[i].cate_name+"</span>";
                        }
                        if(res[i].add_child)
                        {
						...
						if(cate_ids.indexOf(','+res[i].cate_id+',')>0)
						{
                        str+="<tr class='"+pid+" row"+id+"'><td class='align_center w30'><input type='checkbox' class='checkitem' value='"+res[i].cate_id+"' checked/></td>";
						}
						else
						{
                        ...
                        str+="<td class='node' width='50%'>"+itd2+"</td>";
			if(guanli=="all")
			{
            str+="<td class='align_center'><span class='editable' ectype='inline_edit' fieldname='sort_order' fieldid='"+res[i].cate_id+"' datatype='number'>"+res[i].sort_order+"</span></td>";
			}
           // str+="<td class='align_center'>"+if_show+"</td>";
			if(guanli=="all")
			{
            str+="<td class='handler'><span><a href='index.php?app=gcategory&amp;act=edit&amp;id="+res[i].cate_id+"'>"+lang.edit+"</a>  | <a href='javascript:if(confirm(\""+lang.confirm_delete+"\"))window.location =\"index.php?app=gcategory&amp;act=drop&amp;id="+res[i].cate_id+"\";'>"+lang.drop+"</a>" + add_child + "</span></td></tr>";
			}
                    }
                    pr.after(str);
                    change_background();
