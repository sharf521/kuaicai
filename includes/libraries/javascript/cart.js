function drop_cart_item(store_id, rec_id){
	...
	var am_jifen_span = $('#cart' + store_id + '_am_jifen');
    ...
    $.getJSON('index.php?app=cart&act=drop&rec_id=' + rec_id, function(result){
        if(result.done){
            ...
            /*else{
                tr.remove();        //移除
                amount_span.html(price_format(result.retval.amount_m));  //刷新总费用
				am_jifen_span.html(result.retval.am_jifen);  //刷新总费用
...
function change_quantity(store_id, rec_id, spec_id, input, orig){
    ...
    var amount_span = $('#cart' + store_id + '_amount');
	//var amount_m_span = $('#cart' + store_id + '_amount_m');
	 var subjifen_span = $('#item' + rec_id + '_subjifen');
    var am_jifen_span = $('#cart' + store_id + '_am_jifen');
    ...
    $.getJSON('index.php?app=cart&act=update&spec_id=' + spec_id + '&quantity=' + _v, function(result){
        if(result.done){
            ...
            subtotal_span.html(price_format(result.retval.subtotal_m));
           //amount_span.html(price_format(result.retval.amount));
			amount_span.html(price_format(result.retval.amount_m));
			subjifen_span.html(result.retval.subjifen);
            am_jifen_span.html(result.retval.am_jifen);
        }
        else{
            //更新失败
            alert(result.msg);
