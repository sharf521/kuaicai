...
class BaseOrder extends Object
{
    ...
    function _get_shipping_methods($store_id)
    {
        ...
        $shipping_model =& m('shippings');

        return $shipping_model->find('store_id=' . $store_id);
    }
	...
	$cityrow=$this->_city_mod->get_cityrow();
		...
		if($goods_info['daishou']==3)
		{
			$goods_amount_m=$goods_info['amount'];	
		}
		else
		{
			$goods_amount_m= m_21($goods_info['amount']);	
		}
        /* 返回基本信息 */
        return array(
			...
			'goods_jifen'  =>  $goods_info['am_jifen'],
            'discount'      =>  isset($goods_info['discount']) ? $goods_info['discount'] : 0,
			'youhuidiscount'      =>  isset($goods_info['youhuidiscount']) ? $goods_info['youhuidiscount'] : 0,
			'discount_jifen'      =>  isset($goods_info['discount_jifen']) ? $goods_info['discount_jifen'] : 0,
			'youhuidiscount_jifen'      =>  isset($goods_info['youhuidiscount_jifen']) ? $goods_info['youhuidiscount_jifen'] : 0,
			'coupon_id'     =>  $goods_info['coupon_id'],
			'coupon_sn'     =>  $goods_info['coupon_sn'],
			'bianhao'     =>  $goods_info['bianhao'],
			'youhui_id'     =>  $goods_info['youhui_id'],
			'youhui_name'     =>  $goods_info['youhui_name'],
            'anonymous'     =>  intval($post['anonymous']),
            'postscript'          =>  trim($post['postscript']),
			'city'          =>  $city_id,
			'daishou'          =>  $goods_info['daishou'],
        );
    ...
    function _handle_consignee_info($goods_info, $post)
    {
        ...
        /* 计算配送费用 */
        $shipping_model =& m('shippings');
       /* $shipping_info  = $shipping_model->get("shipping_id={$consignee_info['shipping_id']} AND store_id={$goods_info['store_id']} ");
        if (empty($shipping_info))
        {
            $this->_error('no_such_shipping');
            return false;
        }
$user_id=$goods_info['store_id'];//卖家


foreach($goods_info['items'] as $key=>$val)
{
	$userid=$val['user_id'];//买家
}

$member_row=$shipping_model->getRow("select * from ".DB_PREFIX."member where user_id='$userid' limit 1");*/
...
{
	$userid=$val['user_id'];//买家
	$zong_feemoney+=$val['fee_money'];
	$zong_feejifen+=$val['fee_jifen'];
}
        return array(
