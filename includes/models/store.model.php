...
class StoreModel extends BaseModel
{
    ...
    function get_info($store_id)
    {
        $info = $this->get(array(
            ...
            'fields'     => 'this.*,member.user_name, member.email,member.level',
        ));
...
$credit_store=$this->store_mod->getRow("select store_jifen from ".DB_PREFIX."store where store_id='$store_id' limit 1");
$store_jifen=$credit_store['store_jifen'];
		...
		$new_credit_value=$credit_value+$store_jifen;
    ...
    function compute_credit($credit_value, $step = 5)
    {
        ...
        if ($credit_value < 100)
        {
            return 'heart_' . (floor(($credit_value ) / 20) + 1) . '.gif';
        }
        elseif ($credit_value < 5000)
        {
            return 'diamond_' . (floor(($credit_value ) / 1000) + 1) . '.gif';
        }
