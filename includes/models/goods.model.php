...
class GoodsModel extends BaseModel
{
    ...
    function get_list($params = array(), $scate_ids = array(), $desc = false, $no_picture = true)
    {
        ...
        $fields = "g.goods_id, g.store_id, g.type, g.goods_name, g.cate_id, g.cate_name, g.brand, g.spec_qty,g.price_m, g.spec_name_1, g.spec_name_2, g.if_show, g.closed, g.add_time, g.recommended, g.default_image, g.cityhao,g.daishou,g.is_ershou,g.erweima,g.shipping_id,g.subhead, " .
                "gs.spec_id, gs.spec_1, gs.spec_2, gs.color_rgb, gs.price, gs.stock,gs.jifen_price,gs.vip_price, " .
...
class GoodsBModel extends GoodsModel
{
	...
	'daishou'=>2,
	'spec_qty'=>$goo['spec_qty'],
	'tags'=>$goo['tags']
	);		
	$goods_id =$this->_goods_mod->add($add_goods); 
	//$this->_goods_mod->edit($var, array('gong_id'=>$goods_id));
	//增加商品category_goods表
	$add_cate=array(
	'cate_id'=>0,
	'goods_id'=>$goods_id,
	...
	$spec_id=$this->_spec_mod->add($add_spec);
	
	$gsp=$this->_goods_mod->getAll("select gsp.* from ".DB_PREFIX."goods_spec gsp " .
				" where gsp.goods_id = '$var' and gsp.spec_id!='$defaultspec'");
	if($gsp)
	{
		foreach($gsp as $key=>$va)
		{
			$add_spec=array(
			'goods_id'=>$goods_id,
			'price'=>$va['price'],
			'price_m'=>$va['price_m'],
			'jifen_price'=>$va['jifen_price'],
			'vip_price'=>$va['vip_price'],
			'stock'=>$va['stock'],
			'sku'=>$va['sku'],
			'spec_1'=>$va['spec_1'],
			'spec_2'=>$va['spec_2']
			);
			$this->_spec_mod->add($add_spec);
		}
	}	
	...
	foreach($gim as $tp)
	{
		...
		$file_id=$this->_uploadedfile_mod->add($add_file);
		//增加goods_image表
		$add_image=array(
		'goods_id'=>$goods_id,
