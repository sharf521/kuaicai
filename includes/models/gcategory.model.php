...
class GcategoryModel extends BaseModel
{
     ...
     * @return array
     */
    function get_list($parent_id = -1, $shown = false,$qiantai=0,$fen=0)
    {
	...
	$userid=$_SESSION['admin_info']['user_id'];
      ...
      // $parent_id >= 0 && $conditions .= " AND parent_id = '$parent_id'"; 
		...
		if ($qiantai==0)
		{
			...
			{
			$parent_id >= 0 && $conditions .= " AND parent_id = '$parent_id'"; 
			$conditions .= " AND  (city='$city'  or city=0 or city is null)";
			}
		}
		else
		{
		 ...
		 if($fen!=2)
		 {
		// $conditions .= " AND  (city='$city_id'  or city=0 or city is null)";
		 $conditions .= " and siteids like '%|$city_id,%' ";
		 }
...
class GcategoryBModel extends GcategoryModel
{
     ...
     * 判断名称是否唯一
     */
    function unique($cate_name, $parent_id, $cate_id = 0,$city=0)
    {
        $conditions = "parent_id = '$parent_id' AND cate_name = '$cate_name'";
		$city && $conditions .= " and (city='$city' or city='')";
    ...
    function get_children($id, $cached = false)
    {
        ...
        if ($cached)
        {
            $data = $this->_get_structured_data(2);
            if (isset($data[$id]))
            {
                $cids = $data[$id]['cids'];
                foreach ($cids as $id)
     ...
     */
    function _get_structured_data($fen=0)
    {
        ...
        if ($data === false)
        {
            ...
            $cate_list = $this->get_list(-1, true,1,$fen);
