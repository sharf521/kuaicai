...
class Find_passwordApp extends MallbaseApp
{
    ...
    function Find_passwordApp()
    {
		...
		$this->_zfpassword_mod = &m("my_money"); 
    }
    ...
    function index()
    {
       ...
       else
       {
           ...
           if (empty($info) || $info['email'] != $email)
           {
			...
			sendmail(addslashes($mail['subject']),addslashes($mail['message']),$email);
...
//商付通支付密码修改 增加部分 开始***********************************************************
    function set_zf_password()
    {
        ...
        else
        {
            ...
            $ret = $this->_password_mod->edit($id, array('activation' => $md5word));//再随机产生一个
