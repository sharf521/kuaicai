...
class CashierApp extends ShoppingbaseApp
{
    ...
    function index()
    {
        ...
        if (!$order_info['payment_id'])
        {
			...
			//商付通 V2.2 BY:xiaohei  QQ:77491010  读取资金，读取店铺是否安装支付方式
			$my_money_model =& m('my_money');//model
			$this->kaiguan_mod =& m('kaiguan');		
			
			$user_id=$this->visitor->get('user_id');
			$my_money=$my_money_model->getRow("select * from ".DB_PREFIX."my_money where user_id='$user_id' limit 1");
			$this->assign('money', $my_money);
			$kaiguan=$this->kaiguan_mod->getRow("select * from ".DB_PREFIX."kaiguan limit 1");
			$this->assign('kaiguan', $kaiguan);
			
			$canshu=$this->kaiguan_mod->getRow("select zhe_jifen from ".DB_PREFIX."canshu limit 1 ");
			
			$order_info['dazhe_jifen']=$canshu['zhe_jifen']*$order_info['order_jifen'];
			$canshu['zhe_jifen']=$canshu['zhe_jifen']*10;
			$this->assign('canshu',$canshu);
			
			
			$seller_id=$order_info['seller_id'];
			$sft=$payment_model->getRow("select * from ".DB_PREFIX."payment where store_id='$seller_id' and enabled=1 and payment_code='sft'");
			$this->assign('sft', $sft);
			//商付通 结束
