...
class My_shippingApp extends StoreadminbaseApp
{
    ...
    function drop()
    {
		...
		$row=$model_shipping->getAll("select if_show from ".DB_PREFIX."goods where shipping_id='$shipping_id' and store_id= ".$this->visitor->get('manage_store'));
		foreach($row as $key=>$var)
		{
			if($var['if_show']==1)
			{
				$this->show_warning('cipeisong');
				return;
			}
		}
		
		 $drop_count = $model_shipping->drop("store_id = " . $this->visitor->get('manage_store') . " AND shipping_id='$shipping_id' " );
