...
class CouponApp extends StoreadminbaseApp
{
    ...
    function add()
    {
        ...
        else
        {
			...
			$coupon_jifen = floatval(trim($_POST['coupon_jifen']));
            $use_times = intval(trim($_POST['use_times']));
            $min_amount = floatval(trim($_POST['min_amount']));
			$min_jifen = floatval(trim($_POST['min_jifen']));
            ...
            if ($end_time < $start_time)
            {
			 ...
			 if ($min_amount <= $coupon_value)
            {
                $this->pop_warning('gouwudayu');
                exit;
            }
			
			$city_row=$this->canshu_mod->getRow("select city from ".DB_PREFIX."member where user_id = '$this->_store_id' limit 1");
			$city=$city_row['city'];
            $coupon = array(
                'coupon_name' => trim($_POST['coupon_name']),
                'coupon_value' => $coupon_value,
				'coupon_jifen' => $coupon_jifen,
    ...
    function _mail_to_user($users, $coupon)
    {
        foreach ($users as $val)
        {
           ...
           // $this->_mailto($val['email'], addslashes($mail['subject']), addslashes($mail['message']));
			sendmail(addslashes($mail['subject']),addslashes($mail['message']),$val['email']);
        }
    }
