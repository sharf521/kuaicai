...
class ApplyApp extends MallbaseApp
{
    ...
    function index()
    {
		...
		$useid=$this->visitor->get('user_id');
	$row_member=$this->member_mod->getRow("select * from ".DB_PREFIX."member where user_id = '$useid' limit 1");
        ...
        switch ($step)
        {
            ...
            case 2:
                ...
                if (!IS_POST)
                {
				 ...
				 $mem=$region_mod->getRow("select real_name,owner_card from ".DB_PREFIX."member where user_id='$userid' limit 1");
		$this->assign('mem', $mem); 
                    /* 导入jQuery的表单验证插件 */
			...
			if(!trim($_POST['owner_name']))
    		{
                $this->show_message('zhanghubuneng','go_back','index.php?app=member&act=profile'); 
                return;
    		}
			if(!trim($_POST['owner_card']))
    		{
                $this->show_message('sfzbuneng','go_back','index.php?app=member&act=profile'); 
                return;
    		}
					...
					if (empty($image))
					{
						$this->show_warning('zhengjianbunengweikong');
						return;
					}
