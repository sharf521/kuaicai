...
class Seller_groupbuyApp extends StoreadminbaseApp
{
    ...
    function Seller_groupbuyApp()
    {
		...
		$this->goods_mod =& bm('goods', array('_store_id' => $this->_store_id));
    }
    ...
    function add()
    {
	   ...
	   $spec_row=$this->spec_mod->getRow("select stock from ".DB_PREFIX."goods_spec where goods_id='$goodsid'");	
	$stock=$spec_row['stock'];

	$canshu=$this->member_mod->can();
    $baozhengjin=$canshu['tg_baozhengjin'];//保证金
    $gb_fei=$canshu['tg_fei'];//发起团购费用
    $zgb_fei=$baozhengjin+$gb_fei;
	$money_row=$this->spec_mod->getRow("select money,suoding_money from ".DB_PREFIX."my_money where user_id='$user_id' limit 1");	
	$money=$money_row['money'];
	$suoding_money=$money_row['suoding_money'];
	$keyong_money=$money-$suoding_money;

	if($keyong_money<$zgb_fei)
	{
	  $this->show_message('nindezhanghuyuebuzu',
                'back_list', 'index.php?app=seller_groupbuy');
		return;
	}
	/*if($stock<$min_quantity)
	{
	$this->show_message('bunengfaqituangou',
                'back_list', 'index.php?app=seller_groupbuy');
		return;
	}*/
