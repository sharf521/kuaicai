...
class FrontendApp extends ECBaseApp
{
	...
	$jie=$this->jiekuan_mod->getrow("select * from ".DB_PREFIX."jiekuan where user_id = '$user_id' and status=2 and status1=1");
	 $jieshu_time=substr($jie['jieshu_time'],0,10);
	 if($jie)//判断是否有符合条件的借款记录
	 {
		$user_name=$this->visitor->get('user_name');
		$riqi=date('Y-m-d');
		if($riqi>$jieshu_time)//判断是否逾期
		{
			...
			$this->my_money_mod=& m('my_money');
			 ...
			 $zonglixi=$zong_lixi+$faxi;
			 $yinghuan_money=$money_j-$money_j*1/10-$money_yh+$zong_lixi;//应还金额

			 $result=$this->jiekuan_mod->getrow("select * from ".DB_PREFIX."my_money where user_id = '$user_id'");
			 $city=$result['city'];
			...
			if($yinghuan_money <= $new_moneyh)
			{
				$data=array('money_h'=>$new_moneyh,'end_time'=>$shi,'status1'=>2,'faxi'=>$zonglixi);
			...
			{
			    $data=array('money_h'=>$new_moneyh,'end_time'=>$riqi,'status1'=>1,'faxi'=>$zonglixi);
	...
	//$city_id=$row_city['city_id'];
	//$huiyuan=$row_city['huiyuan'];
	$cityrow=$this->_city_mod->get_cityrow();
	$city_id=$cityrow['city_id'];
	$huiyuan=$cityrow['huiyuan'];
/*	echo $row_city['upfile'];*/
	//$logo=$this->_city_mod->getAll("select upfile from ".DB_PREFIX."city where city_yuming = '$url'");
...
if($weiboid!="" && $openid=="")
{
	$dd=array('weiboid'=>$weiboid);
}
if($weiboid!="" && $openid!="")
{
	$dd=array('weiboid'=>$weiboid,'openid'=>$openid);
}
...
$qd=$db->getrow("select * from ".DB_PREFIX."qiandao where user_id='$user_id'");
$can=$db->getrow("select * from ".DB_PREFIX."canshu");
           ...
           /* $menu['im_seller']['submenu']['my_payment'] =  array(
            ...
            $menu['im_seller']['submenu']['coupon']  = array(
			 ...
			 $menu['im_seller']['submenu']['gonghuo']  = array(
                    ...
                    'url'   => 'index.php?app=seller_groupbuy&act=gonghuo',
                    'name'  => 'gonghuo',
                    'icon'  => 'ico20',
            );
			 $menu['im_seller']['submenu']['mycaigou']  = array(
                    'text'  => Lang::get('mycaigou'),
                    'url'   => 'index.php?app=seller_groupbuy&act=my_caigou',
                    'name'  => 'mycaigou',
                    'icon'  => 'ico6',
            );
...
class StorebaseApp extends FrontendApp
{
	...
	$userid=$this->_store_id;
	 $mfbb=$this->member_mod->getrow("select level from ".DB_PREFIX."member where user_id=$userid");
    $eve=$mfbb['level'];
	$le=level($eve);
        ...
        if ($store === false)
        {
			...
			$st=$this->member_mod->getrow("select erweima from ".DB_PREFIX."store where store_id='$this->_store_id'");
			$this->assign('st',$st);
			if($st['erweima']==1)
			{
