    ...
    function uploadedfile()
    {
           ...
           // $remain     = $settings['space_limit'] > 0 ? $settings['space_limit'] * 1024 * 1024 - $upload_mod->get_file_size($this->store_id) : false;
            ...
            if ($files['error'] === UPLOAD_ERR_OK)
            {
                ...
                $dirname = '';
				$_shopid=$this->visitor->get('manage_store');
				if ($this->belong == BELONG_GOODS)
                {
                    $dirname = 'data/files/'.intval($_shopid/2000).'/shop_' . $_shopid . '/' . date('Ym');//'/goods_' . (time() % 200)
                }
                elseif ($this->belong == BELONG_STORE)
                {
                    $dirname = 'data/files/'.intval($_shopid/2000).'shop_' . $_shopid . '/other';
                }
                elseif ($this->belong == BELONG_ARTICLE)
                {
                    $dirname = 'data/files/'.intval($_shopid/2000).'shop_' .$_shopid.'/article';
                }
				...
				{
					echo "<script type='text/javascript'>alert('{$arr['error']}');</script>";
					$this->view_iframe();
                    return false;
				}	
