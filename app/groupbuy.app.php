...
class GroupbuyApp extends StorebaseApp
{
    ...
    function index()
    {
		...
		$canshu=$this->_groupbuy_mod->can();
		$kaiguan=$this->_groupbuy_mod->kg();
		$this->assign('kaiguan',$kaiguan);
		$sql="select * from ecm_order_goods og,ecm_order o,ecm_groupbuy g where g.goods_id=og.goods_id and og.order_id=o.order_id and g.goods_id='$group[goods_id]' and o.extension='groupbuy' and o.add_time>=g.start_time and o.add_time<=g.end_time";
		
			$grou=$this->_groupbuy_mod->getAll($sql);
			$group['coun']=count($grou);
...
$member_mod->unlinkRelation('join_groupbuy', $this->_visitor['user_id'], $group['group_id']);
        ...
        if (!IS_POST)
        {
            ...
            foreach ($goods['_specs'] as $key => $spec)
            {
                ...
                else
                {
					...
					$goods['_specs'][$key]['group_price_m'] = $group['spec_price'][$spec['spec_id']]['price']*(1+$canshu['lv21']);
					$goods['_specs'][$key]['tgjifen_price'] = $group['spec_price'][$spec['spec_id']]['price']*$canshu['jifenxianjin']*(1+$canshu['lv31']);
					$goods['_specs'][$key]['tgvip_price'] = $group['spec_price'][$spec['spec_id']]['price']*$canshu['jifenxianjin']*(1+$canshu['lv21']);
					$goods['_specs'][$key]['fen_price']=$goods['_specs'][$key]['price']*$canshu['jifenxianjin']*(1+$canshu['lv31']);
        ...
        else
        {
                if (isset($_POST['join']))
                {
                   ...
                   // $this->show_message('join_groupbuy_successed');
					//$this->show_message('join_groupbuy_successed','back_list','index.php?app=order&goods=groupbuy&group_id='.$id);
                ...
                header("Location:index.php?app=order&goods=groupbuy&group_id=$id");
                    return;
                }
    ...
    function _ican($id, $state, $store_id, $act = '')
    {
        ...
        if ($this->_visitor['user_id'] > 0) //已登录用户
        {
            // 是否已经参加
        ...
        {
            $state_permission[GROUP_ON] = array('ask', 'join', 'login'); // login提示需要登录才能参加
        }
