    ...
    function cancel_order()
    {
	 ...
	 $this->message_mod=& m('message');
		...
		sendmail(addslashes($mail['subject']),addslashes($mail['message']),$seller_info['email']);
			...
			$sell=$this->model_member->getrow("select user_name from ".DB_PREFIX."member where user_id='$seller_id'");
...
/* by:xiaohei QQ:77491010 商付通 更新商付通定单状态 开始******************************************************/
	...
	$buyer_user_name=$my_moneylog_row['buyer_name'];//买家ID
	$sell_user_name=$my_moneylog_row['seller_name'];//卖家ID
	
	$riqi=date('Y-m-d H:i:s');
	$city=$my_moneylog_row['city'];
	 $buy_vip=$this->order_mod->getrow("select vip from ".DB_PREFIX."member where user_id='$buyer_user_id' limit 1");//买家
	 $buy_vip=$buy_vip['vip'];
	/* if($buy_vip==1)
	 {
		 $sell_jifen=$order_jifen/(1+$lv21);//普通商品卖家得到的积分、返还价
	 }
	 else
	 {
		 $sell_jifen=$order_jifen/(1+$lv31);
	}*/
	...
	//推荐奖励
	$this->order_mod->tuijian($sell_jifen,$buyer_user_id,$sell_user_id,$order_sn,$order_jifen);
	if($is_gh==1)
	{
	...
	if($my_moneylog_row['order_id']=$order_id)
	{
		...
		$buy_money_row=$this->my_money_mod->getrow("select * from ".DB_PREFIX."my_money where user_id='$buyer_user_id'");
		...
		if($is_gh!=1)
		{
			...
			$new_money = $sell_money+$order_amount;
			$beizhu1=Lang::get('dingdanhao').$order_sn;
			//$new_duihuanjifen = $sell_duihuanjifen+$jifen_zhesuan;
			$new_duihuanjifen = $sell_duihuanjifen+$sell_jifen;
		}
		

		
//买家对接开始
			if($webservice=="yes" && $zhifufangshi=="jifenzhifu")
			{
				$post_data = array(
				"ID"=>$buy_web_id,
				//"Money"=>$jifen_zhesuan,
...
//更新数据
		...
		if($is_gh==1)//更新供货人的金钱或积分
		{
			if(in_array(1,$bb))//若是付费商家
			{
				...
				{
					//$gonghuo_jifen=$jifen_zhesuan*(1-$daishou_bili);
					$gonghuo_jifen=$sell_jifen*(1-$daishou_bili);
					$new_gonghuo_jifen=$gong_u_jf_duihuan+$gonghuo_jifen;
        			//$notice=$sell_user_name.Lang::get('dongjiejifen').$dongjiejifen.Lang::get('jifen').Lang::get('geinin');
					$new_gong_money_array=array(
					'duihuanjifen'=>$new_gonghuo_jifen
					);
					//添加供货人的moneylog
					//$beizhu=$gong_username.Lang::get('shoudaojiaoyi').$gonghuo_jifen.Lang::get('jifen');
					$beizhu=Lang::get('dingdanhao').$order_sn;
					$gong_money=array(
					//'jifen_dj'=>'-'.$gonghuo_jifen,
					'jifen'=>$gonghuo_jifen,
					'time'=>$riqi,
					...
					's_and_z'=>1,
					'beizhu'=>$beizhu,
					'dq_money'=>$gong_u_money,
					'dq_money_dj'=>$gong_u_moeny_dj,
					'dq_jifen'=>$new_gonghuo_jifen,
				...
				'zong_jifen'=>$account_zongjifen-$gonghuo_jifen
		  		...
		  		else
		  		{
				    ...
				    $new_gonghuo_money=$gong_u_money+$gonghuo_money;
					
        		//$notice=$sell_user_name.Lang::get('dongjiejifen').$dongjiejifen.Lang::get('jifen').Lang::get('geinin');
		  
					$new_gong_money_array=array(
					'money'=>$new_gonghuo_money
					);
					//添加供货人的moneylog
					//$beizhu=$gong_username.Lang::get('shoudaojiaoyixianjin').$gonghuo_money.Lang::get('yuan');
					$beizhu=Lang::get('dingdanhao').$order_sn;
					$gong_money=array(
					//'money_dj'=>'-'.$gonghuo_money,
					'money'=>$gonghuo_money,
					'time'=>$riqi,
					'user_name'=>$gong_username,
					'user_id'=>$gong_userid,
					...
					's_and_z'=>1,
					'beizhu'=>$beizhu,
					'dq_money'=>$new_gonghuo_money,
					'dq_money_dj'=>$gong_u_moeny_dj,
					'dq_jifen'=>$gong_u_jf_duihuan,
					'dq_jifen_dj'=>$gong_u_jf_dongjie	
					);
					
					/*$beizhu1 =Lang::get('congzhanghuxianjin');
					$beizhu1=str_replace('{3}',$buyer_user_name,$beizhu1);
				    $beizhu1=str_replace('{4}',$gonghuo_money,$beizhu1);
					$beizhu1=str_replace('{5}',$gong_username,$beizhu1);*/
				$beizhu1=Lang::get('dingdanhao').$order_sn;
				$addaccount=array(
					'money'=>'-'.$gonghuo_money,
					'time'=>$riqi,
			...
			$this->canshu_mod->edit('id=1',$edit_account);
				 ...
				 //$notice=str_replace('{2}',$jifen_zhesuan,$notice);
				$notice=str_replace('{2}',$sell_jifen,$notice);
				 ...
				 $add_notice=array(
					...
					$this->message_mod->add($add_notice);
					//$this->my_money_mod->edit('user_id='.$sell_user_id,array('yingde_jifen'=>$sell_yingde_jifen+$jifen_zhesuan));//添加卖家的累计应得积分
			   ...
			   $ri=date('Y-m-d H:i:s');
			    if($webservice=="yes")//卖家对接webservice
			    {
					$post_data = array(
					"ID"=>$sell_web_id,
					//"Money"=>$jifen_zhesuan,//订单积分
					"Money"=>$sell_jifen,//订单积分
					"MoneyType"=>1,
					"Count"=>1
					); 
			...
			else//若是免费商家
			{
			   ...
			   $shouyi_jifen_count=0;
				...
				if($zhifufangshi=="jifenzhifu")
				{
					if($shouyi_jifen_count<0)
					{
						$shouyi_jifen_count=0;
					}
					//$gong_shouyi=$jifen_zhesuan-$shouyi_jifen_count;//供货人的收益
					$gong_shouyi=$sell_jifen-$shouyi_jifen_count;//供货人的收益
					//$log_text=Lang::get('kou').$gong_shouyi.Lang::get('jifen').Lang::get('gei').$gong_username;
					...
					'gonghuoshang'=>$gong_username
					 ...
					 $new_account_zongmoney=$account_zongmoney;
					$gong_shouyi_array=array(
						'duihuanjifen'=>$gong_u_jf_duihuan+$gong_shouyi
					);					
					//添加供货人的moneylog
					//$beizhu=$gong_username.Lang::get('shoudaojiaoyi').$gong_shouyi.Lang::get('jifen');
					$beizhu=Lang::get('dingdanhao').$order_sn;
				...
				else
				{
				    ...
				    $gong_shouyi_money=$order_amount-$shouyi_money;//供货人的收益
					//$log_text=Lang::get('kou').$gong_shouyi_money.Lang::get('yuan').Lang::get('gei').$gong_username;
					$log_text=Lang::get('dingdanhao').$order_sn;
					$addaccount=array(
					...
					'gonghuoshang'=>$gong_username
					 ...
					 $new_account_zongjifen=$account_zongjifen;
					...
					//添加供货人的moneylog
					//$beizhu=$gong_username.Lang::get('shoudaojiaoyixianjin').$gong_shouyi_money.Lang::get('yuan');
					$beizhu=Lang::get('dingdanhao').$order_sn;
					$gong_shouyi_array_log=array(					
						'money'=>$gong_shouyi_money,
						'time'=>$riqi,
	     ...
	     { 
			if($zhifufangshi=="jifenzhifu")
			{
				...
				//添加moneylog日志
				//$beizhu=$sell_username.Lang::get('shoudaojiaoyi').$jifen_zhesuan.Lang::get('jifen');
				$sell_money=array(
				//'jifen'=>$jifen_zhesuan,
				'jifen'=>$sell_jifen,
				'time'=>$riqi,
				...
				$addaccount=array(
					...
					'beizhu'=>$log_text,
					'dq_money'=>$account_zongmoney,
					//'dq_jifen'=>$account_zongjifen-$jifen_zhesuan
					'dq_jifen'=>$account_zongjifen-$sell_jifen,
					'xiaofei'=>$buyer_user_name,
					'shangjia'=>$sell_username,
					'gonghuoshang'=>$gong_username
				);			
				$edit_account=array(	
				//'zong_jifen'=>$account_zongjifen-$jifen_zhesuan
				'zong_jifen'=>$account_zongjifen-$sell_jifen
				);
			...
			else
			{
				...
				//添加moneylog日志
				//$beizhu=$sell_username.Lang::get('shoudaojiaoyixianjin').$order_amount.Lang::get('yuan');
				$sell_money=array(
				//'money_dj'=>'-'.$money,
				'money'=>$order_amount,
				'time'=>$riqi,
				...
				$log_text=Lang::get('dingdanhao').$order_sn;
					...
					's_and_z'=>2,
					'beizhu'=>$log_text,
					'dq_money'=>$account_zongmoney-$order_amount,
					'dq_jifen'=>$account_zongjifen,
					'xiaofei'=>$buyer_user_name,
					'shangjia'=>$sell_username,
					'gonghuoshang'=>$gong_username
					...
					$edit_account=array(	
				              'zong_money'=>$account_zongmoney-$order_amount
				              );
...
//增加积分
$buyer_jifen_row=$this->my_money_mod->getrow("select * from ".DB_PREFIX."my_money where user_id='$buyer_user_id'");
...
{
$new_jifen=$jifen+$order_amount;
$new_jifen_z=$jifen_z+$order_amount;
}
...
//	$city_id=$row_city['city_id'];
	$cityrow=$this->_city_mod->get_cityrow();
        ...
        $order_info  = $model_order->get("order_id={$order_id} AND buyer_id=" . $this->visitor->get('user_id'));
		$seller_id=$order_info['seller_id'];
		$result1=$model_order->getrow("select user_name from ".DB_PREFIX."member where user_id='$seller_id'");
...
$riqi=date('Y-m-d H:i:s');
	...
	function tousu()
	{
	 ...
	 $this->order_mod =& m('order');     
	 $order_id = isset($_GET['order_id']) ? intval($_GET['order_id']) : 0;
	$ts=$this->order_mod->getrow("select * from ". DB_PREFIX ."order  where order_id='$order_id'");
...
//$comp=$this->order_mod->getrow("select * from ". DB_PREFIX ."complain  where order_id='$order_id'");
	...
	if($_POST)
	{
	 ...
	 $mm=$this->order_mod->getrow("select * from ". DB_PREFIX ."member  where user_id='$respondent_id'");
	...
	function bei_tousu()
	{
	 ...
	 $this->_curlocal(LANG::get('member_center'),    'index.php?app=member',
            ...
            /* 当前用户中心菜单 */
			 ...
			 $user_id=$this->visitor->get('user_id');
			 $this->complain_mod=& m('complain');
$btou=$this->complain_mod->getAll("select * from ". DB_PREFIX ."complain  where respondent_id='$user_id' and ts_id=0"); 
   ...
   $com=$this->complain_mod->getrow("select * from ".DB_PREFIX."complain where id = '$id'");
   $recid=$com['rec_id'];
$or=$this->complain_mod->getAll("select * from ".DB_PREFIX."order_goods where order_id = '$order_id' and rec_id in ($recid) ");
  ...
  function ts_drop()
	...
	function shensu()
	{
	 ...
	 $ts=$this->complain_mod->getrow("select * from ". DB_PREFIX ."complain  where id='$id'");
	 $riqi=date('Y-m-d H:i:s');
//$comp=$this->order_mod->getrow("select * from ". DB_PREFIX ."complain  where order_id='$order_id'");
	if($_POST)
	{
	 ...
	 $id=trim($_POST['id']);//申诉id
	  $da = array(
            ...
            /* 当前用户中心菜单 */
			 ...
			 $shs=$this->complain_mod->getrow("select * from ". DB_PREFIX ."complain  where ts_id='$id'"); 
...
$btou=$this->complain_mod->getAll("select * from ". DB_PREFIX ."complain  where respondent_id='$user_id' and ts_id!=0"); 
   ...
   $com=$this->complain_mod->getrow("select * from ".DB_PREFIX."complain where id = '$id'");
   $ts_id=$com['ts_id'];
   $tous=$this->complain_mod->getrow("select * from ".DB_PREFIX."complain where id = '$ts_id'");
