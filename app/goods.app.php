...
class GoodsApp extends StorebaseApp
{
	...
	$mem=$this->_goods_mod->getRow("select vip from ".DB_PREFIX."member where user_id='$user_id' limit 1");
       
	    $kaiguan=$this->_goods_mod->kg();
		$canshu=$this->_goods_mod->can();
		
		$cityrow=$this->_goods_mod->get_cityrow();
		$city_id=$cityrow['city_id'];

        /* 参数 id */
        $id = empty($_GET['id']) ? 0 : intval($_GET['id']);
	$row=$this->_goods_mod->getRow("select store_id,erweima from ".DB_PREFIX."goods where goods_id='$id' limit 1");
	$store_id=$row['store_id'];
        ...
        /* 更新浏览次数 */
        $this->_update_views($id);//商品
		$this->store_views($store_id);//店铺
		...
		if($row['erweima']==1)
		{
		$imgsrc=qrcode("http://".$_SERVER['HTTP_HOST']."/index.php?app=goods&id=".$id,"./data/files/store_$store_id/goods_$id/",$city_id.'erweima.png');
		$qrcc="<img src='$imgsrc' width='125'>";
		}
		
        $this->assign('kaiguan',$kaiguan);
		$this->assign('mem',$mem);
		$this->assign('qrcc',$qrcc); 
    ...
    function qa()
    {
        ...
        $goods_qa =& m('goodsqa');
         $id = intval($_GET['id']);
		$row_good=$this->_goods_mod->getRow("select * from ".DB_PREFIX."goods where goods_id = '$id' limit 1");
	    $city=$row_good['cityhao'];
	/*	echo $city;*/
    ...
    function _get_common_info($id)
    {
       ...
       // $key = 'page_of_goods_' . $id;
        ...
        if ($data === false)
        {
			...
			$canshu=$this->_goods_mod->can();
			$goods['baoyou_jifen']=$goods['baoyou_money']*$canshu['jifenxianjin'];
    ...
    function _get_sales_log($goods_id, $num_per_page)
    {
        ...
        $sales_list = $order_goods_mod->find(array(
            ...
            'fields'=> 'buyer_id, buyer_name, add_time, anonymous, goods_id, specification, price,price_m, quantity, evaluation,jifen,order_alias.zhifufangshi',
            'count' => true,
            'order' => 'add_time desc',
            'limit' => $page['limit'],
        ));
