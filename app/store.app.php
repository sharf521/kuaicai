...
class StoreApp extends StorebaseApp
{
    ...
    function groupbuy()
    {
        ...
        foreach ($groupbuy_list as $key => $_g) {
            ...
            $groupbuy_list[$key]['pricem'] = $tmp['price'] * (1 + $canshu['lv21']);
    ...
    function credit()
    {
        ...
        $goods_list = $order_goods_mod->find(array(
            ...
            'fields' => 'buyer_id, buyer_name, anonymous, evaluation_time, goods_id, goods_name, specification, price,jifen, quantity, goods_image, evaluation, comment,zhifufangshi,price_m',
    ...
    function _get_recommended_goods($id, $num = 12)
    {
        ...
        $goods_list = $goods_mod->find(array(
           // 'conditions' => 'closed = 0 AND if_show = 1 AND recommended = 1 and daishou!=2 and cityhao=' . $city_id,
            'conditions' => 'closed = 0 AND if_show = 1 AND recommended = 1 ',
            'fields' => 'goods_name, default_image, price,jifen_price,vip_price,store_id,price_m,daishou,subhead ',
            'order' => 'add_time desc',
            'limit' => $num,
        ));
        foreach ($goods_list as $key => $goods) {
            empty($goods['default_image']) && $goods_list[$key]['default_image'] = Conf::get('default_goods_image');
            $goods_list[$key]['price_m'] = round($goods['price_m'], 2);
    ...
    function _get_new_goods($id, $num = 12)
    {
        $goods_mod =& bm('goods', array('_store_id' => $id));
        $goods_list = $goods_mod->find(array(
            //'conditions' => "closed = 0 AND if_show = 1 and daishou!=2",
