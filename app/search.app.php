...
//$row_city=$this->_city_mod->getrow("select * from ".DB_PREFIX."city where city_yuming = '$url'");
	...
	$daishou=$_GET['daishou'];
		...
		if($orde=='')
		{
			$sql="SELECT *,g.goods_id, case g.cityhao 
...
*/	
		...
		foreach ($goods_list as $key => $goods)
        {
			...
			$goods_list[$key]['region']=substr($goods['region_name'],5,100);
        }
		...
		$daishou = isset($param['daishou']) ? $param['daishou'] : 0;

		if($cate_id==0)
		{
		$this->_curlocal($this->get_goods_curl($recom_id,$is_ershou,$daishou));
		}
		else
		{
...
/*$colect=$model_goods->getrow("select count(*) from ".DB_PREFIX."collect where item_id='$storeid' and type='store'");
$stores[$key]['count']=$colect['count'];*/
            //等级图片
            $step = intval(Conf::get('upgrade_required'));
            $step < 1 && $step = 5;
...
$this->assign('kaiguan',$kaiguan);
        ...
        foreach ($groupbuy_list as $key => $groupbuy)
        {
			...
			$groupbuy_list[$key]['pricem'] = $groupbuy['spec_price'][$groupbuy['default_spec']]['price']*(1+$canshu['lv21']);
        }
    ...
    function _recommended_groupbuy($_num)
    {
        ...
        foreach ($data as $gb_id => $gb_info)
        {
			...
			$data[$gb_id]['pricem']      = $price['price']*(1+$canshu['lv21']);
...
function guanggaowei($_num,$type)
    ...
    function _get_goods_curlocal($cate_id)
    {
	 ...
	 function get_goods_curl($recom_id,$is_ershou=0,$daishou=0)
    {
...
$rec=$this->city_mod->getRow("select * from ".DB_PREFIX."recommend where recity = '$cityid' and recom_id=$recom_id");
		...
		else
		{
			 $curlocal = array(
            array('text' => LANG::get('all_categories'), 'url' => "/index.php?app=category"),
			array('text' => LANG::get('gonghuoliebiao')),
        );
		}
    ...
    function _get_query_param()
    {
		...
		if (isset($_GET['daishou']) && intval($_GET['daishou']) > 0)
        {
            $res['daishou'] = intval($_GET['daishou']);
        }
...
if (isset($param['jifen_price']))
	...
	$city_id=$cityrow['city_id'];
		...
		if (isset($param['daishou']))
        {
            $conditions .= " AND g.daishou = '" . $param['daishou'] . "'";
        }
		else
		{
			$conditions .= " AND g.daishou != 2";
		}
        if (isset($param['price']))
        {
            $min = $param['price']['min'];
    ...
    function _get_group_by_info($param, $cached,$recom_id="")
    {
        ...
        if ($data === false)
        {
            ...
            if ($total_count > 0)
            {
				...
				foreach($data['by_region'] as $key=>$val)
				{
					$data['by_region'][$key]['region_name']=substr($val['region_name'],5,100);
				}
                /* 按价格统计 */
                if ($total_count > NUM_PER_PAGE)
                {
                    $sql = "SELECT MIN(g.price_m) AS min, MAX(g.price_m) AS max FROM {$table} WHERE" . $conditions;
                    ...
                    $sql = "SELECT FLOOR((g.price_m - '$min') / '$step') AS i, count(*) AS count FROM {$table} WHERE " . $conditions . " GROUP BY i ORDER BY i";
                    ...
                    while ($row = $goods_mod->db->fetchRow($res))
                    {
                        $data['by_price'][] = array(
                            'count' => $row['count'],
                            'min'   => round($min + $row['i'] * $step,0),
                            'max'   => round($min + ($row['i'] + 1) * $step,0),
                        );
                    }
                }
				//按积分价格查询
