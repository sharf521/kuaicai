...
function main() 
{
    if (!mxClient.isBrowserSupported()) {
        ...
        $.ajax({
            type: "post",
            url: "ajax.php",
            data: { func:'getJDTreeJson',user_id:user_id,user_name:user_name,plevel:plevel,nlevel:nlevel,pan:pan},
            dataType: "json",
            ...
            success: function (data) {
                ...
                try {
                    ...
                    for (var i = 0; i < data.length; i++) {
                            ...
                            if (i == 0) {
                                var vNode = graph.insertVertex(parent, data[i]['UserID'], data[i]['user_name']+'\r\n'+data[i]['user_id'], w / 3 - 30, 20, 60, 40);
                                node[data[i]['ID']] = vNode;
                            } else {
                                vNode = graph.insertVertex(parent, data[i]['UserID'], data[i]['user_name']+'\r\n'+data[i]['user_id'], 0, 0, 60, 40);
                                node[data[i]['ID']] = vNode;
                            }
                    ...
                    for (var j = 0; j < data.length; j++) {
                            ...
                            if (j != 0) {
                                graph.insertEdge(parent, null, '', node[data[j]['Parent']], node[data[j]['ID']]);
                            }
