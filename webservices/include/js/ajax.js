...
function getAjaxTable(url) {
	createXMLHttpRequest();
	if (document.getElementById('div_id_id')){
		cle_div();
	}
...
function createTabel(atop,aleft){
	...
	div.setAttribute( "id" , "div_id_id"); //层的唯一id
	sub_id = "sub_div"; //
	...
	var   div_sub_sub   =   document.createElement("DIV"); //创建div层
	div_sub_sub.style.zIndex= 100;
	div_sub_sub.style.position = "absolute";
	document.body.appendChild(div_sub_sub);
...
{
	var obj = document.getElementById("div_id_id");
	var sha = obj.nextSibling;
	var obj_sub = document.getElementById("div_sub_sub");
