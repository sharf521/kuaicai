<?php
	...
	{	
		exit();
		$result=$db->get_all("select a.user_id FROM  `ecm_member` a JOIN ecm_my_webserv b ON a.user_id = b.user_id  where b.paymoney!=5800 and LOCATE(  '2',  a.level)<1");
		foreach($result as $row)
		{
			$user_id=$row['user_id'];
			$sql="update {member} set level=0 where user_id=$user_id limit 1";
			echo $sql.'<br>';
			$db->query($sql);
		}
		$result=null;
		/*$result=$db->get_all("select user_id,user_name,password,email,last_ip,last_login from {member}");
		foreach($result as $row)
		{
			echo "INSERT INTO `uc_members` (uid,`username`, `password`, `email`, `myid`, `myidkey`, `regip`, `regdate`, `lastloginip`, `lastlogintime`, `salt`, `secques`) VALUES ('".$row['user_id']."','".$row['user_name']."', '".md5($row['password'])."', '".$row['email']."', '', '', '".$row['last_ip']."', '".$row['last_login']."', 0, 0, '', '');<br>";
   ...
   header("Content-Disposition: attachment; filename=".$savename.".$file_ending");      
	...
	elseif($func=='vip_up')
	{
		...
		$sql="select b.user_id,sum(a.Mony) as zengjin from {process} a join {member} b on a.UserID=b.web_id where a.PlateNum>1 group by b.user_id";
		$result=$db->get_all($sql);
		foreach($result as $row)
		{
			$user_id=$row['user_id'];
			$zengjin=(int)$row['zengjin'];
			$db->query("update {my_money} set zengjin=$zengjin where user_id=$user_id limit 1");	
		}
		$result=null;
		echo '更新完成!';
	}
	elseif($func=='jiesuan')
	{
		...
		$enddate=date("Y-m-d",strtotime("+1days",strtotime($date)));
		//$sqlW="a.status=0 and a.IncomeTime>='".($date)."' and a.IncomeTime<='".$date." 23:59:59'";
		$sqlW="a.status=0 and a.IncomeTime>='".($date)."' and a.IncomeTime<='".$enddate."'";	
		...
		foreach($result as $row)
		{
			...
			//更改增进数据状态
			$db->query("update {process} a set status=1 where a.UserID='".$row['UserID']."' and $sqlW and a.PlateNum>1");	
		}
		$result=null;
		
		
		//$db->query("update {process} a set status=1 where  $sqlW and a.PlateNum>1");	//更改增进数据状态	
		...
		$sql="select b.user_id,max(a.PlateNum) as t from {process} a join {member} b on a.UserID=b.web_id where a.IncomeTime>='".($date)."' and a.IncomeTime<='".$date." 23:59:59' and a.PlateNum>1 group by b.user_id";
		//有增进的数据才更新
		$result=$db->get_all($sql);
		foreach($result as $row)
		{
		...
		//更改其它平台数据
		//$db->query("update {process} a set status=2 where status=0 and $sqlW ");
		echo '完成!';
		exit();	
	}
	...
	{	
		...
		if($type==1)//fbb
		{
			$url="select max(ProcessId) proid from {process} where Aside1=4";
			$c='GetFListInfo';
		}
		elseif($type==2)//z100
		{
			$url="select max(ProcessId) proid from {process} where Aside1=5";
			$c='GetZListInfo';
		}
		elseif($type==3)
		{
			$url="select max(ProcessId) proid from {process} where Aside1=6";
			$c='GetLListInfo';
		}
		elseif($type==4)//1.2.3商城返利
		{
			$url="select max(ProcessId) proid from {process} where Aside1<4";	
			$c='GetMListInfo';
		}
		...
		$result=webService($c,array('Start'=>$proid,'Num'=>500));
		$count=count($result);
		if($count==0)
		{
			echo '接收完成';
		...
		{
			//$result=$result['Process'];//每个类型返回的数据格式不一样
			...
			if(!empty($result['ProcessID']))
			{
				...
				if($result['Aside1']>3)//fbb,100,zeng..钱转积分
				{
					...
					$result['IncomeTime']=date("Y-m-d H:i:s",strtotime($result['IncomeTime'])+60*60*24);
				}
				//$result['Mony']=((int)($result['Mony']*10000))/10000;//保留4位小数
	//			unset($result['ProcessID']);
			...
			else
			{
				...
				foreach($result as $v)
				{
					...
					if($v['Aside1']>3)//fbb,100,zeng..钱转积分
					{
						...
						$v['IncomeTime']=date("Y-m-d H:i:s",strtotime($v['IncomeTime'])+60*60*24);	
					}
					//unset($v['ProcessID']);
					$db->insert('{process}',$v);
					$i++;
...
//		$db->query("TRUNCATE TABLE  {member}");	
	...
	elseif($func=='get_mallrebates')
	{
		$data=array();
		$url='http://'.$_S['canshu']['webservip'].'/connstr.asp';
